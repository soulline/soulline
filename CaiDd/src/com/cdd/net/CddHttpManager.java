...
public class CddHttpManager {
	...
	private static CddHttpManager manager = null;
	...
	private class SJMDHttpRequestThread extends HandlerThread implements
		...
		private CddHttpClient client = null;
		...
		public boolean handleMessage(Message msg) {
			...
			if (what == REQUEST_WHAT) {
				...
				client = new CddHttpClient(context);
	...
	private CddHttpManager(Context context) {
	...
	public static CddHttpManager getInstance(Context context) {
		if (manager == null) {
			manager = new CddHttpManager(context);
		}
		return manager;
	}
	...
	private void request(HttpUriRequest request, CddHttpClient client,
			Request runnable) {
		InputStream is = null;
		...
		try {
			...
			if (code == HttpStatus.SC_OK) {
				...
				CddException ze = new CddException(code + "");
				runnable.callBack.onFetch(false, ze);
			}
		} catch (Exception e) {
			CddException ze = new CddException(e);
			runnable.callBack.onFetch(false, ze);
			e.printStackTrace();
		} finally {
			try {
